...
from generalized_alphanpi.core.PolicyOnly import PolicyOnly
...
if __name__ == "__main__":
    ...
    if args.to_stdout:
        print(
            f"{method},{dataset},{np.mean(reward)},{1 - np.mean(reward)},{sum(costs) / len(costs)},{np.std(costs)},{sum(length_actions) / len(length_actions)},{np.std(length_actions)}")
