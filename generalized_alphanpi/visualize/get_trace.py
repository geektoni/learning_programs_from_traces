...
class MCTSvisualiser:
    ...
    def _print_mcts_edge(self, parent, child, color=None):
        edge_label = self.env.get_program_from_index(child.program_from_parent_index)+"("+str(child.args)+")"
        self._print_edge(parent.index, child.index, label=edge_label, color=color)
